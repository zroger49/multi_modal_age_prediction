...
# @Description: Code to run differential gene expression analysis (using only common samples from the multi-modal dataset)
...
for (tissue in tissues){
  ...
  # Folds were created in scripts/3.feature_selection/08_genera_folds_multi_omics.ipynb
  for (fold in all_folds){
    ...
    if (fold != "train"){
      ...
      exclusive_samples <- read.csv(paste0("../../results/3.feature_selection_multimodal/", tissue_name_2, "/exclusive_gene_expression_train.csv"))
    ...
    model_function <- function(mod, mod_no_age){
      ...
      saveRDS(residuals, paste0("../../results/3.feature_selection_multimodal/results/", tissue, dir_res, "/gexp_residuals.rds")) #Correcting for covariates without Age
      
      
      #Cor with age (residuals)
      cor_with_age <- cor(t(residuals), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection_multimodal/results/", tissue, dir_res, "/cor_age_residuals_gexp.csv"), quote = F, row.names = T)
      
      #Cor with Age (logTPM values)
      cor_with_age <- cor(t(logTPM_train), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection_multimodal/results/", tissue, dir_res, "/cor_age_values_gexp.csv"), quote = F, row.names = T)
      ...
      write.csv(res_M, paste0("../../results/3.feature_selection_multimodal/results/", tissue, dir_res, "/DEG_results.csv"), quote = F, row.names = T)
