...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('9.3_gexp_downsample_EN_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    def objective(trial, X):   
        ...
        for cv_train_samples, cv_valid_samples in folds: 
            ...
            pipe = Pipeline([
                ...
                ("elasticNet", ElasticNet(alpha=search_params["alpha"], l1_ratio=search_params["l1_ratio"], tol = 0.001)) # Increase tol a bit due to converge warning
            ])
    ...
    #study.trials_dataframe().to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/3_optuna_table.csv")
    ...
    for cv_train_samples, cv_valid_samples in folds: 
        ...
        pipe = Pipeline([
            ("QuantileTransformer", QuantileTransformer(n_quantiles = 100, random_state=SEED, output_distribution = "normal")),
            ("elasticNet", ElasticNet(alpha=best_params["alpha"], l1_ratio=best_params["l1_ratio"], tol = 0.001)) 
        ])
        if best_params["use_lds"] == True: 
            pipe.fit(X_fold_train, y_fold_train, elasticNet__sample_weight = w)
        else: 
    ...
    with mlflow.start_run(run_name = tissue + "_elastic_net_multi_modal_gexp_downsample", experiment_id = EXPERIMENT_ID) as _:
        ...
        plot_model_fit(y_train, 
                        y_train_pred, 
                        data_set="Train", 
                        fig_output_path= f"aging_notes/figures/9.multi_modal_models_gexp_hist/3_{tissue}_EN_gexp_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models_gexp_hist/3_{tissue}_EN_gexp_fit_test.pdf")
        ...
        model_coef.to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/3_EN_gexp_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models_gexp_hist/{tissue}/3_EN_gexp.pipeline.pkl', compress = 1)
