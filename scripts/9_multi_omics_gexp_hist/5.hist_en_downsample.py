"""Run EN on each tissue using features from histology
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('9,5_hist_EN_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/5_optuna_table.csv")
    ...
    with mlflow.start_run(run_name = tissue + "_elastic_net_hist_downsample", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/5_hist_en_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/5_hist_en_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                        y_train_pred, 
                        data_set="Train", 
                        fig_output_path= f"aging_notes/figures/9.multi_modal_models_gexp_hist/5_{tissue}_EN_hist_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models_gexp_hist/5_{tissue}_EN_hist_fit_test.pdf")
        ...
        model_coef.to_csv(f"results/9.multi_modal_models_gexp_hist/{tissue}/5_EN_hist_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models_gexp_hist/{tissue}/5_EN_hist.pipeline.pkl', compress = 1)
