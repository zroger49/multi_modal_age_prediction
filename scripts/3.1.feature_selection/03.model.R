...
for (tissue in tissues){
  ...
  if (!dir.exists(paste0("../../results/3.feature_selection/results/", tissue))){
    dir.create(paste0("../../results/3.feature_selection/results/", tissue), recursive = T)
  }
  ...
  for (fold in c(0,1,2,3,4, "train")){
    ...
    if (fold != "train"){
      ...
      if (!dir.exists(paste0("../../results/3.feature_selection/results/", tissue, dir_res))){
        dir.create(paste0("../../results/3.feature_selection/results/", tissue, "/fold_", fold), recursive = T)
      }
    }else{
      dir_res <- paste0("/train")
      if (!dir.exists(paste0("../../results/3.feature_selection/results/", tissue, dir_res))){
        dir.create(paste0("../../results/3.feature_selection/results/", tissue, dir_res), recursive = T)
      }
    }
    ...
    if (fold != "train"){
      ...
      fold_data <- read.csv(paste0("../../results/3.feature_selection/", tissue_name_2, "/fold_", fold, "_train.csv"))
    ...
    model_function <- function(mod, mod_no_age){
      ...
      #saveRDS(residuals, paste0("../../results/3.feature_selection/results/", tissue, dir_res, "/methylation_residuals.rds")) #Correcting for covariates without Age
      #Cor with age (residuals)
      cor_with_age <- cor(t(residuals), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      #write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection/results/", tissue, dir_res, "/cor_age_residuals.csv"), quote = F, row.names = T)
      
      #Cor with Age (Methylation values)
      cor_with_age <- cor(t(M), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      #write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection/results/", tissue, dir_res, "/cor_age_M_values.csv"), quote = F, row.names = T)
      ...
      #write.csv(res_M, paste0("../../results/3.feature_selection/results/", tissue, dir_res, "/DML_results.csv"), quote = F, row.names = T)
    }
