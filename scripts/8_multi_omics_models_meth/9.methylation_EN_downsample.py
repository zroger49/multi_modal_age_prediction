"""Run EN on each tissue using features from methylation using the same sample size as the multi_modal dataset. 
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('9_meth_hist_EN_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_model/{tissue}/3_optuna_table.csv")
    ...
    with mlflow.start_run(run_name = tissue + "_elastic_net_multi_meth_hist", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/3_meth_hist_en_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/3_meth_hist_en_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                       y_train_pred, 
                       data_set="Train", 
                       fig_output_path= f"aging_notes/figures/9.multi_modal_models/3_{tissue}_EN_hist_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models/3_{tissue}_EN_hist_fit_test.pdf")
        
    
        #Save coefficients
        coef = pipe[1].coef_
        model_coef =  pd.DataFrame({"probe":features_to_keep, "coef" : coef})       
        model_coef.to_csv(f"results/9.multi_modal_models/{tissue}/3_EN_meth_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models/{tissue}/3_EN_meth.pipeline.pkl', compress = 1)
