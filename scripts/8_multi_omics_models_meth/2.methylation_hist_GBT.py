"""Run GBT on each tissue using features from methylation and histology. 
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('2_meth_hist_EN_LDS.log'),
    ]
)
...
#EXPERIMENT_ID  = mlflow.create_experiment(f"multi_modal_models_GBT")
...
for tissue in tissues_to_run:
    ...
    #Split in Train and test data (Only methylation). Histological data will be added later
    logging.info("Spliting methylation data into train and test data..")
    X_train, X_test, y_train, y_test = split_in_train_test(meth_data_t, complete_samples, test_set)
    ...
    def objective(trial, X):   
        ...
        #complete_samples_age_test = complete_samples_age[complete_samples_age_train.sample_id.isin(test_set["sample_id"])]
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_model/{tissue}/2_optuna_table.csv")
    ...
    with mlflow.start_run(run_name = tissue + "_GBT_multi_modal_meth_hist", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/2_meth_hist_en_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/2_meth_hist_en_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                       y_train_pred, 
                       data_set="Train", 
                       fig_output_path= f"aging_notes/figures/9.multi_modal_models/2_{tissue}_EN_hist_meth_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models/2_{tissue}_EN_hist_meth_fit_test.pdf")
        
    
        #Save coefficients
        importance_gain = pipe[1].feature_importances_
        probes = features_to_keep.extend(hist_data_df_train.columns)
        model_coef =  pd.DataFrame({"probe":probes, "coef" : importance_gain})

        model_coef.to_csv(f"results/9.multi_modal_models/{tissue}/2_EN_hist_meth_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models/{tissue}/2_EN_hist_meth.pipeline.pkl', compress = 1)
