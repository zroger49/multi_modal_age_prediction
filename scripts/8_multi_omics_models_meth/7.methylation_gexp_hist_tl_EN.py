"""Run EN on each tissue using features from methylation, gexp, histology and telemores 
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('7_meth_hist_gexp_tl_EN_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    # Subset meth, gexp and tl dataset
    ...
    # Combine dataset (Gexp, Methylation and telemores. Histology will be added later)
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_models/{tissue}/9_optuna_table.csv")
    ...
    for cv_train_samples, cv_valid_samples in folds: 
        ...
        # Feature selection methylation (always use DML)
        feature_folder_name = tissue_param[tissue].get("name2", tissue_param[tissue]["name"])
        feature_file = "results/3.feature_selection_multimodal/results/" + feature_folder_name + "/fold_" + str(idx) + "/DML_results.csv"
        features_dataset = pd.read_csv(feature_file)
        features_to_keep_meth = features_dataset[features_dataset["adj.P.Val"] < 0.05]
    ...
    with mlflow.start_run(run_name = tissue + "_elastic_net_multi_modal_meth_gexp_hist_tl.rerun", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/9_meth_hist_en_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/9_meth_hist_en_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                        y_train_pred, 
                        data_set="Train", 
                        fig_output_path= f"aging_notes/figures/9.multi_modal_models/9_{tissue}_EN_hist_meth_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models/9_{tissue}_EN_hist_meth_fit_test.pdf")
        ...
        model_coef.to_csv(f"results/9.multi_modal_models/{tissue}/9_EN_hist_meth_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models/{tissue}/9_EN_hist_meth.pipeline.pkl', compress = 1)
