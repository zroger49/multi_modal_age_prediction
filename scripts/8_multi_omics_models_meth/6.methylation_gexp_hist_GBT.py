...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('6_meth_hist_GexpGBT_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_models/{tissue}/8_optuna_table.csv")
    ...
    with mlflow.start_run(run_name = tissue + "_GBT_multi_modal_meth_gexp_hist", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/8_meth_hist_en_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/8_meth_hist_en_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                        y_train_pred, 
                        data_set="Train", 
                        fig_output_path= f"aging_notes/figures/9.multi_modal_models/8_{tissue}_EN_hist_meth_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models/8_{tissue}_EN_hist_meth_fit_test.pdf")
        ...
        model_coef.to_csv(f"results/9.multi_modal_models/{tissue}/8_EN_hist_meth_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models/{tissue}/8_EN_hist_meth.pipeline.pkl', compress = 1)
