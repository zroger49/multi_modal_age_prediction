...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('4_meth_hist_EN_LDS.log'),
    ]
)
...
for tissue in tissues_to_run:
    ...
    def objective(trial, X):   
        ...
        for cv_train_samples, cv_valid_samples in folds: 
            ...
            # Feature selection for Meth data (This is a parameter to optimize)
    ...
    study.trials_dataframe().to_csv(f"results/9.multi_modal_models/{tissue}/6_optuna_table.csv")
    ...
    with mlflow.start_run(run_name = tissue + "_GBT_multi_modal_meth_gexp", experiment_id = EXPERIMENT_ID) as _:
        ...
        train_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/6_meth_gexp_GBT_train_prediction.csv")
        ...
        test_prediction.to_csv(f"results/9.multi_modal_models/{tissue}/6_meth_gexp_GBT_test_prediction.csv")
        ...
        plot_model_fit(y_train, 
                        y_train_pred, 
                        data_set="Train", 
                        fig_output_path= f"aging_notes/figures/9.multi_modal_models/6_{tissue}_GBT_gexp_meth_fit_train.pdf")


        plot_model_fit(y_test, 
                    y_test_pred, 
                    data_set="Test", 
                    fig_output_path= f"aging_notes/figures/9.multi_modal_models/6_{tissue}_GBT_gexp_meth_fit_test.pdf")


        #Save coefficients
        importance_gain = pipe[1].feature_importances_
        features = features_to_keep_meth
        model_coef =  pd.DataFrame({"features":features, "coef" : importance_gain})       
        model_coef.to_csv(f"results/9.multi_modal_models/{tissue}/6_GBT_gexp_meth_feature_importance.csv")

        # Save model 
        joblib.dump(pipe, f'results/9.multi_modal_models/{tissue}/6_GBT_gexp_meth.pipeline.pkl', compress = 1)
