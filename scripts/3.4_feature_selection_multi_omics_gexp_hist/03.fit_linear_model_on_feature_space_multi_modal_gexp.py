...
This script runs for Gexp + histology"""
...
def split_in_train_test_gexp(gexp, age_data, test_set):
...
for tissue in tissues_to_run:
    ...
    # Load multi-omics dataset to subset age_data
    ...
    age_data = age_data.loc[age_data["tissue_sample_id"].isin(common_samples["sample_id"])]
    ...
    X_train, X_test, y_train2, y_test = split_in_train_test_gexp(gexp_data, age_data, test_set)
    ...
    exclusive_gexp_samples = pd.read_csv(f'results/3.feature_selection_gexp_hist/{tissue}/exclusive_gene_expression_train.v2.csv', index_col=0)
    ...
    for cv_train_idx, cv_valid_idx in folds: 
        ...
        model_coef.to_csv(f"results/3.feature_selection_gexp_hist/results/{tissue_name}/fold_{idx}/linear_model_coef_gexp.csv")
        idx += 1
    ...
    model_coef.to_csv(f"results/3.feature_selection_gexp_hist/results/{tissue_name}/train/linear_model_coef_gexp.csv")
