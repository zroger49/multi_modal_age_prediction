...
# @Description: Code to run differential gene expression analysis (using only common samples from Hist + Geme expression)
...
for (tissue in tissues){
  ...
  if (!dir.exists(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue))){
    dir.create(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue), recursive = T)
  }
  ...
  # Folds were created in scripts/3.feature_selection/11.generate_folds_multi_omics_hist_gexp.ipynb
  for (fold in all_folds){
    cat(paste0("Fold ", fold, "\n"))
    
    if (fold != "train"){
      dir_res <- paste0("/fold_", fold)
      if (!dir.exists(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res))){
        dir.create(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, "/fold_", fold), recursive = T)
      }
    }else{
      dir_res <- paste0("/train")
      if (!dir.exists(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res))){
        dir.create(paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res), recursive = T)
      }
    }
    ...
    if (fold != "train"){
      ...
      fold_data <- read.csv(paste0("../../results/3.feature_selection_gexp_hist/", tissue_name_2, "/fold_", fold, "_train.csv"))
      exclusive_samples <- read.csv(paste0("../../results/3.feature_selection_gexp_hist/", tissue_name_2, "/exclusive_gene_expression_train.v2.csv"))
      samples <- c(gsub("-SM-.*", "", fold_data$sample), exclusive_samples$tissue_sample_id)
      metadata <- metadata %>% filter(ID %in% samples)
    }else{
      #Remove the test set samples (Note, I use the complete train set here, and not only common samples)
    ...
    logTPM_train <- logTPM %>% select(metadata$ID)
    ...
    model_function <- function(mod, mod_no_age){
      ...
      saveRDS(residuals, paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res, "/gexp_residuals.rds")) #Correcting for covariates without Age
      
      
      #Cor with age (residuals)
      cor_with_age <- cor(t(residuals), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res, "/cor_age_residuals_gexp.csv"), quote = F, row.names = T)
      
      #Cor with Age (logTPM values)
      cor_with_age <- cor(t(logTPM_train), metadata_2$AGE, method = "spearman")
      colnames(cor_with_age) <- "rho"
      write.csv(cor_with_age, file =  paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res, "/cor_age_values_gexp.csv"), quote = F, row.names = T)
      ...
      write.csv(res_M, paste0("../../results/3.feature_selection_gexp_hist/results/", tissue, dir_res, "/DEG_results.csv"), quote = F, row.names = T)
